...
import { CircleX, HelpCircle, CheckCircle2, Clock, XCircle, AlertCircle } from "lucide-react";
...
export default function ConferenceRegistration() {
  ...
  const { data: conference, isLoading: conferenceLoading } = api.member.registration.getLatestConference.useQuery();
  ...
  if (conference?.registrationStartDate && conference?.registrationEndDate) {
    isRegistrationOpen = conference.registrationStartDate <= now && conference.registrationEndDate >= now;
  }
  ...
  return (
    <div className="flex-1 space-y-6 p-3 sm:p-4 md:p-6">
        ...
        {conferenceLoading ? (
          ...
          <>
            ...
            {conference.startDate && conference.endDate && (
              <p className="text-base opacity-80">
                Conference Date: {new Date(conference.startDate).toLocaleDateString()} - {new Date(conference.endDate).toLocaleDateString()}
              </p>
            )}
            {conference.registrationStartDate && conference.registrationEndDate && (
              <p className="text-base opacity-80">
                Registration Period: {new Date(String(conference.registrationStartDate)).toLocaleDateString()} - {new Date(String(conference.registrationEndDate)).toLocaleDateString()}
              </p>
            )}
          </>
        ) : (
          <>
            ...
            <p className="mt-2 text-sm opacity-75">
              Please check back later or contact the association for more information
            </p>
          </>
        )}
      </div>
          ...
          <CardContent>
            <div className="flex flex-col items-center max-w-md mx-auto text-center">
              ...
              <p className="text-red-600 dark:text-red-400">
                Registration is currently not open. Please check the registration period above or contact the association for assistance.
              </p>
            </div>
          </CardContent>
        </Card>
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <CardDescription className="mt-2">
                      Submitted on {new Date(existingRegistration.createdAt).toLocaleDateString()} at{" "}
                      {new Date(existingRegistration.createdAt).toLocaleTimeString()}
                    </CardDescription>
                  </div>
                </div>
              </CardHeader>
              <CardContent className="space-y-6">
                ...
                {existingRegistration.status === "pending" && (
                  <div className="flex items-start gap-3 rounded-lg border border-yellow-200 bg-yellow-50 p-4 dark:border-yellow-900 dark:bg-yellow-950">
                    <AlertCircle className="text-yellow-600 dark:text-yellow-400 mt-0.5 h-5 w-5 flex-shrink-0" />
                    <div>
                      <h3 className="font-semibold text-yellow-900 dark:text-yellow-100">
                        Registration Under Review
                      </h3>
                      <p className="text-muted-foreground mt-1 text-sm">
                        Your registration is currently being reviewed by the administrators. You will be
                        notified once your registration is approved. Please check back later.
                      </p>
                    </div>
                  </div>
                )}
                ...
                {existingRegistration.status === "confirmed" && (
                  <div className="flex items-start gap-3 rounded-lg border border-green-200 bg-green-50 p-4 dark:border-green-900 dark:bg-green-950">
                    <CheckCircle2 className="text-green-600 dark:text-green-400 mt-0.5 h-5 w-5 flex-shrink-0" />
                    <div>
                      <h3 className="font-semibold text-green-900 dark:text-green-100">
                        Registration Confirmed!
                      </h3>
                      <p className="text-muted-foreground mt-1 text-sm">
                        Your registration has been approved. Please ensure payment is completed before the
                        conference date.
                      </p>
                    </div>
                  </div>
                ...
                {existingRegistration.status === "cancelled" && (
                  <div className="flex items-start gap-3 rounded-lg border border-red-200 bg-red-50 p-4 dark:border-red-900 dark:bg-red-950">
                    <XCircle className="text-red-600 dark:text-red-400 mt-0.5 h-5 w-5 flex-shrink-0" />
                    <div>
                      <h3 className="font-semibold text-red-900 dark:text-red-100">
                        Registration Cancelled
                      </h3>
                      <p className="text-muted-foreground mt-1 text-sm">
                        This registration has been cancelled. If you believe this is an error, please
                  ...
                  <div className="grid gap-4 md:grid-cols-2">
                    ...
                    <div>
                      ...
                      <p className="font-medium">
                        ...
                        {((existingRegistration.priceCents ?? 0) / 100).toFixed(2)}
                      </p>
                    </div>
                    ...
                    <div>
                      ...
                      <div className="grid gap-4 md:grid-cols-2">
                        ...
                        <div>
                          ...
                          <p className="font-medium">
                            {new Date(existingRegistration.conference.startDate).toLocaleDateString()} -{" "}
                            {new Date(existingRegistration.conference.endDate).toLocaleDateString()}
                          </p>
                        </div>
                      </div>
                    </div>
              ...
              <Card id="bank-details-section" className="border-primary bg-blue-50 dark:bg-gradient-to-r dark:from-[#93E3F2] dark:from-25% dark:to-[#00AEE8] dark:to-100%">
                ...
                <CardContent className="text-black">
                  <div>
                    ...
                    <span className="ml-2">
                      {conference ? `${conference.currency} $${(conference.priceCents / 100).toFixed(2)}` : 'TBD'}
                    </span>
                  </div>
                  <Separator
                  ...
                  <div>
                    ...
                    <span className="ml-2">{conference?.bankTransferAccountName ?? 'TBD'}</span>
                  </div>
                  <div>
                    <span className="font-semibold">Bank & Branch:</span>
                    <span className="ml-2">{conference?.bankTransferBranch ?? 'TBD'}</span>
                  </div>
                  <div>
                    <span className="font-semibold">Account Number:</span>
                    <span className="ml-2">{conference?.bankTransferAccountNumber}</span>
                  </div>
                </CardContent>
              </Card>
                  ...
                  {conference?.contacts && conference.contacts.length > 0 ? (
                    <>
                      {conference.contacts.map((contact, index: number) => (
                        <RegistrationContactRow contact={contact} index={index} key={index} />
                      ))}
                    </>
                  ) : (
      ...
      {!existingRegistration && <div className="relative grid h-full grid-cols-1 gap-6 lg:grid-cols-3">
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                ...
                <div className="space-y-3">
                  ...
                  >
                    <div className="flex items-center space-x-2">
                      <RadioGroupItem value="levy" id="levy" disabled={!isRegistrationOpen} />
                      <Label htmlFor="levy" className="text-sm">
                        Levy of {conference ? `${conference.currency} $${(conference.priceCents / 100).toFixed(2)}` : 'FJD $250'} (crossed cheque)
                      </Label>
                    </div>
                    <div className="flex items-center space-x-2">
                      <RadioGroupItem value="deposit" id="deposit" disabled={!isRegistrationOpen} />
              ...
              <CardContent className="space-y-6">
                <div className="space-y-3">
                  ...
                  >
                    <div className="flex items-center space-x-4">
                      <div className="flex items-center space-x-2">
                        <RadioGroupItem value="veg" id="day1-veg" disabled={!isRegistrationOpen} />
                        <Label htmlFor="day1-veg" className="text-sm">
                          Veg
                        </Label>
                      </div>
                      <div className="flex items-center space-x-2">
                        <RadioGroupItem value="non-veg" id="day1-non-veg" disabled={!isRegistrationOpen} />
                ...
                <div className="space-y-3">
                  ...
                  >
                    <div className="flex items-center space-x-4">
                      <div className="flex items-center space-x-2">
                        <RadioGroupItem value="veg" id="day2-conf-veg" disabled={!isRegistrationOpen} />
                ...
                <div className="space-y-3">
                  ...
                  >
                    <div className="flex items-center space-x-4">
                      <div className="flex items-center space-x-2">
                        <RadioGroupItem value="veg" id="day2-closing-veg" disabled={!isRegistrationOpen} />
              ...
              <CardContent className="space-y-4">
                <div className="space-y-4">
                  ...
                  <Button
                    ...
                    disabled={!formData.finalConfirmation || !isRegistrationOpen || conferenceLoading || submitRegistration.isPending}
                  >
                    {conferenceLoading
                      ? "Loading..."
...
function RegistrationContactRow({ contact, index }: { contact: ConferenceContact, index: number }) {
  ...
  return (
    <div key={contact.id || index}>
      {index > 0 && (
        <Separator orientation="horizontal" className="mb-4 bg-[#CCCCCC] dark:bg-white/60" />
      )}
      <div>
