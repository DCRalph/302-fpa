...
export default function EditConferencePage() {
  ...
  // Populate form when conference data is loaded
  useEffect(() => {
    if (conference) {
      setFormData({
        ...
        startDate: new Date(conference.startDate).toISOString().slice(0, 16),
        endDate: new Date(conference.endDate).toISOString().slice(0, 16),
        registrationStartDate: new Date(conference.registrationStartDate).toISOString().slice(0, 16),
        registrationEndDate: new Date(conference.registrationEndDate).toISOString().slice(0, 16),
        priceCents: (conference.priceCents / 100).toString(),
        currency: conference.currency,
        bankTransferAccountName: conference.bankTransferAccountName,
        bankTransferBranch: conference.bankTransferBranch,
        bankTransferAccountNumber: conference.bankTransferAccountNumber,
        maxRegistrations: conference.maxRegistrations.toString(),
  ...
  const updateConferenceMutation = api.admin.conference.update.useMutation({
    onSuccess: (data) => {
      toast.success("Conference updated successfully");
      router.push(`/admin-dashboard/manage-conferences/${data.id}`);
    },
    onError: (err) => {
      toast.error(err.message ?? "Failed to update conference");
    },
  });
    ...
    <main className="flex-1 p-3 sm:p-4 md:p-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="flex gap-4">
          <Button type="submit" disabled={updateConferenceMutation.isPending} className="flex-1">
            {updateConferenceMutation.isPending ? "Updating..." : "Update Conference"}
          </Button>
          <Link href={`/admin-dashboard/manage-conferences/${conferenceId}`} className="flex-1">
