...
export function BenefitsSectionContent({ conferenceJoin: initialConferenceJoin = null }: { conferenceJoin?: ConferenceWhyJoin[] | null }) {
  const conferenceWhyJoin = api.home.getConferenceWhyJoin.useQuery();

  const remoteValue = conferenceWhyJoin.data?.value ?? null;
  const parsedWhyJoin = remoteValue ? JSON.parse(remoteValue ?? "[]") as ConferenceWhyJoin[] : initialConferenceJoin;
