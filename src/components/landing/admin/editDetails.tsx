...
export default function EditDetails() {
  ...
  const { data } = api.home.getConferenceDetails.useQuery(undefined, {
    enabled: open,
  });
  ...
  const { mutate, isPending } = api.admin.editHome.changeConferenceDetails.useMutation({
    onSuccess: async () => {
      await Promise.all([
        utils.home.getConferenceDetails.invalidate(),
      ]);
      setOpen(false);
    },
  });
  ...
  function onSave() {
    try {
      const parsed = JSON.parse(json) as ConferenceDetails;
      mutate(parsed);
    } catch {
    ...
    <Sheet open={open} onOpenChange={setOpen}>
      ...
      <SheetContent side="right">
        <SheetHeader>
          ...
          <SheetDescription>
            Edit the structured details object as JSON.
          </SheetDescription>
        </SheetHeader>

        <div className="p-4 space-y-2">
          <label className="text-sm font-medium" htmlFor="details-json">
            JSON
          </label>
          <textarea
            id="details-json"
